...
addressing. Mops uses A3 and A4 to address the main dictionary &mdash;
...
&lsquo;Dictionary size&rsquo;.)

We call A3 &lsquo;lobase&rsquo; and A4 &lsquo;hibase&rsquo;. Modules are
addressed using A5 (&lsquo;modbase&rsquo;). We never allow a module to
...
The PowerPC has many more registers than the 68k &mdash; 32 integer
...
r13-31 are &lsquo;nonvolatile&rsquo; &mdash; they\'re saved over
...
loop variable \<code\>I\</code\>. \<code\>DO\...LOOP\</code\> will also
be faster if we have the limit value in a register. Now we may as well
use two of our &lsquo;local&rsquo; registers for I and the loop limit,
since it will be very rare for us to need all of them. This will mean
two less locals in definitions that use \<code\>DO\...LOOP\</code\>s,
...
\<code\>DO\...LOOP\</code\> in the definition. This means that if we
...
Next comes the 2-byte handler code (see later under &ldquo;[Compilation
and Optimization](#Compilation_and_Optimization "wikilink")&rdquo;. That
...
evaluated at compile time, and either \<code\>&lt;some code&gt;\</code\>
or \<code\>&lt;some more code&gt;\</code\> will be compiled, but not
...
within \<code\>record {\...}\</code\> won\'t have a header&\#148;that
...
&lsquo;xdesc&rsquo;) to check for the indexed access being in bounds. We
...
negative number as the &ldquo;number of elements&rdquo;, and always
...
&lsquo;development mode&rsquo; these were the only CODE segments there
...
&lsquo;locked&rsquo;. It then couldn\'t expand itself, since it
...
with its new &ldquo;32-bit everything&rdquo; addressing scheme, but in
...
areas. So we have adopted the idea of a &lsquo;segment&rsquo;. A
&lsquo;segment&rsquo; is a contiguous chunk of memory. So the main
...
We use a &lsquo;segment table&rsquo;, which stores the starting address
...
isn\'t a legal value, and give the &ldquo;Not a relocatable
address&rdquo; error. We also give this error on the PowerPC if the
...
If \<code\>saveCfa\</code\> and \<code\>\^obj-mod\</code\> are values or
...
\<code\>BIND\_WITH\</code\> can do a lot to remove the performance
disadvantage of using late binding. See the file Struct for some more
examples, in the \<code\>(Col)\</code\> class.

### Late Bind Cache (PowerMops only) {#late_bind_cache_powermops_only}

This is transparent to users &mdash; it gets used automatically, without
...
gives a useful speedup to many late-bound calls &mdash; especially in
...
exactly the same way as for normal objects and ivars &mdash; that is,
...
incremented. When \<code\>release&gt;\</code\> is done on a reference
...
must be found, and the \<code\>release&gt;\</code\> operation done on
each of these. The reason should be fairly obvious &mdash; since these
...
1.  We go through all our heap blocks and count references to each,
    which come just from other heap blocks. We call this the &ldquo;heap
    reference count&rdquo;. This is easy to compute since as we saw
...
This is basically a form of the traditional &ldquo;mark and sweep&rdquo;
...
\<code\>garbage\_collect\</code\> yourself &mdash; there\'s no problem
...
------------------------------------------------------------------------
  ...
  ------------------------------------------- ----------------------------------- -----------------------------------------
  [Reference 13](Reference_13 "wikilink")     [Reference](Reference "wikilink")   [Reference 15](Reference_15 "wikilink")
